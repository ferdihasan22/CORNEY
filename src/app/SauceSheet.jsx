...
import { SAUCES, fmtRp } from '../data/menu.js'
...
// (they have no sauce). Tanpa batas jumlah saus; saus premium menambah harga.
// Starts from `initial` (sauce ids) — empty for a fresh
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-background rounded-t-3xl p-5 pb-7 shadow-2xl" style={{ animation: 'sheet-up 0.28s ease-out' }}>
        ...
        <div className="flex justify-between items-end mb-4">
          <div className="min-w-0">
            ...
            <p className="font-label-md text-label-md text-on-surface-variant">{subtitle || 'Pilih sesukamu · saus premium menambah harga'}</p>
          </div>
          {picked.length > 0 && <span className="shrink-0 bg-surface-container-high px-3 py-1 rounded-lg font-label-md text-label-md text-primary">{picked.length} dipilih</span>}
        </div>
