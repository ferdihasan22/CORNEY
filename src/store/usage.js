...
export function addUsage({ tgl, branchId, jenis, amount, note }) {
  ...
  const t = new Date()
  commit([{ id: 'U-' + Date.now() + '-' + seq++, tgl, branchId, jenis, amount, note: (note || '').trim(), ts: t.toISOString(), time: t.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }, ...list])
}
export function removeUsage(id) { commit(list.filter((u) => u.id !== id)) }
