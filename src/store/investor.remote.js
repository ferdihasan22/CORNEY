...
export async function pushInvestor(branchId, cfg) {
  ...
  const { error } = await supabase.from('investor_config').upsert({
    branch_id: branchId, sewa: cfg.sewa || 0, gaji: cfg.gaji || 0, value: cfg.value || 0, pct: cfg.pct || 0,
  })
  if (error) console.warn('[investor.write] upsert ' + branchId + ':', error.message || error)
}
