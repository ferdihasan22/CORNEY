...
export async function pushFreezerCell(branchId, parentId, cell) {
  ...
  const { error } = await supabase.from('freezer').upsert(
    { branch_id: branchId, parent_id: parentId, sisa: cell.sisa || 0, min: cell.min || 0, target: cell.target || 0 },
    { onConflict: 'branch_id,parent_id' },
  )
  if (error) console.warn('[freezer.write]', error.message || error)
}
