...
export async function pushFreezerCorrection(c) {
  ...
  const { error } = await supabase.from('freezer_corrections').upsert({
    id: c.id, branch_id: c.branchId, parent_id: c.parent, current: c.current, proposed: c.proposed,
    reason: c.reason, status: c.status, resolved_at: c.resolvedAt || null,
  })
  if (error) console.warn('[fc.write]', error.message || error)
}
