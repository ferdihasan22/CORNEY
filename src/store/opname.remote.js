...
export async function pushOpname(o) {
  ...
  const { error } = await supabase.from('opname').insert({ id: o.id, branch_id: o.branchId, mode: o.mode, rows: o.rows || [], total_selisih: o.totalSelisih })
  if (error) console.warn('[opname.write]', error.message || error)
}
