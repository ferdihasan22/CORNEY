...
export async function pushShipment(s) {
  ...
  const { error } = await supabase.from('shipments').upsert({
    id: s.id, branch_id: s.branchId, parent_id: s.parent, qty: s.qty, status: s.status, selisih: s.selisih || 0,
    confirmed_at: s.confirmedAt || null,
  })
  if (error) console.warn('[shipments.write]', error.message || error)
}
